...
#include "TCPIP.h"
#include "TCPIPDlg.h"
#import "../Debug/Common.tlb" no_namespace named_guids
...
void CTCPIPDlg::OnBnClickedOk()
{
	...
	//
	CoInitialize(NULL);
	HRESULT hr = CoCreateInstance(CLSID_Common, NULL, CLSCTX_INPROC_SERVER, IID_iCommon, reinterpret_cast<void**>(&csharp));

	if (SUCCEEDED(hr))
	{
		
		CString test = csharp->TestPrint();
		csharp->ServerCreate(2001, FALSE);
